...
class Crawler:
    """
    Uma classe para encapsular a lógica do Web Crawler, tornando-o mais organizado
    e evitando o uso de variáveis globais, especialmente importante para multithreading.
    """
    def __init__(self, seed_url, num_threads=10):
        ...
        print(colored(f"[*] Iniciando crawler no domínio: {self.base_domain}", 'cyan'))
        print(colored(f"[*] Usando {self.num_threads} threads.", 'cyan'))
    ...
    def get_links(self, base_url, html):
        """Extrai links do HTML, agora tratando links relativos."""
        links = []
        try:
    ...
    def start(self, output_file=None):
        """Inicia e gerencia as threads do crawler."""
        threads = []
        ...
        print(colored("\n--- Crawling Finalizado ---", 'magenta'))
        print(colored(f"[+] Total de links únicos encontrados: {len(self.found_links)}", 'cyan'))

        if output_file:
            print(colored(f"[*] Salvando resultados em '{output_file}'...", 'blue'))
            with open(output_file, 'w') as f:
                for link in sorted(self.found_links):
                    f.write(link + '\n')
            print(colored("[+] Resultados salvos com sucesso!", 'green'))
